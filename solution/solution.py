...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevant_flags.append(rel >= relevance_threshold)

        num_relevant = sum(1 for flag in relevant_flags if flag)
        if num_relevant == 0:
            return 0.0
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_f = (sum(r.faithfulness for r in new_results) / n) if n else 0.0
        new_r = (sum(r.relevance for r in new_results) / n) if n else 0.0
        new_c = (sum(r.completeness for r in new_results) / n) if n else 0.0

        base_f = (sum(r.faithfulness for r in baseline_results) / b) if b else 0.0
        base_r = (sum(r.relevance for r in baseline_results) / b) if b else 0.0
        base_c = (sum(r.completeness for r in baseline_results) / b) if b else 0.0
